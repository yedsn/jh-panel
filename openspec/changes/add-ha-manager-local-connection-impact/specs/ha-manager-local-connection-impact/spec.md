...
#### Scenario: 连接快照异步加载
- **WHEN** 操作员打开总览页且连接快照尚在采集
- **THEN** 插件在主备、对外服务、自检和云监控等基础总览内容加载完成后一次性展示页面，并仅在连接区域显示采集中状态；连接快照返回后再更新该区域

#### Scenario: 自检异步加载
- **WHEN** 操作员打开总览页且自检尚未完成
- **THEN** 插件先完成主备、对外服务和云监控等基础内容的同步加载并显示总览，自检区域显示“正在自检...”并在 `health_check` 返回后更新

#### Scenario: 基础状态加载骨架屏
- **WHEN** 操作员打开总览页且基础状态或流程配置尚未返回
- **THEN** 插件展示与总览布局对应的标题、操作区和状态行骨架占位；基础数据完成后再整体展示实际总览内容
